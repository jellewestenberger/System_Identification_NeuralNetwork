...
for h=1:size(nrNodesHidden,2)
    if not(size(NNset.IW{h},1)==nrNodesHidden(h)) || not(size(NNset.IW{h},2)== inputs(h))
    ...
    if strcmp(NNset.name,'rbf')
        if not(size(NNset.centers{h},1)==nrNodesHidden(h)) || not(size(NNset.centers{h},2)== inputs(h))
            disp(not(size(NNset.centers{h},1)==nrNodesHidden(h)) || not(size(NNset.centers{h},2)== inputs(h)));
            disp(size(NNset.centers{h},1));
            disp(nrNodesHidden(h))
            disp(size(NNset.centers{h},2));
            inputs(h);
